...
use crate::state::{Config, CONFIG, CW721_ADDRESS, MINTABLE_NUM_TOKENS, MINTED_TOKEN_IDS};
// use crate::state::{MINTED_TOKEN_AMOUNT};
...
pub fn execute_mint_sender(
    ...
    _execute_mint(deps, info, Some(recipient), token_id)
}
...
fn _execute_batch_mint(
    ...
    mut batch_token_ids: Vec<String>,
) -> Result<Response, ContractError> {
    let config = CONFIG.load(deps.storage)?;
    let mintable_num_tokens = MINTABLE_NUM_TOKENS.load(deps.storage)?;
    if mintable_num_tokens == 0 || batch_token_ids.len() > mintable_num_tokens as usize {
        return Err(ContractError::ExceedAmountCanMint {});
    }
    ...
    let mut count: u32 = 0;
    let mut minted_token_ids: Vec<String> = vec![];
    ...
    while let Some(token_id) = batch_token_ids.pop() {
        ...
        if MINTED_TOKEN_IDS.has(deps.storage, token_id.clone()) {
            return Err(ContractError::TokenIdAlreadySold { token_id });
        }

        let msg = _create_cw721_mint(&config, &recipient_addr, token_id.clone());
        ...
        // let minted_token_amount = MINTED_TOKEN_AMOUNT.load(deps.storage)?;

        // Save minted token id to map
        match MINTED_TOKEN_IDS.save(deps.storage, token_id.clone(), &true) {
            Err(_e) => {
                return Err(ContractError::MapSaveError {});
            }
            _ => ()
        }
    ...
    Ok(Response::new()
        ...
        .add_attribute("mintable_num_tokens", mintable_num_tokens.to_string())
        .add_messages(msgs))
}
...
fn _execute_transfer_nft(
    ...
    let mut msgs: Vec<CosmosMsg<Empty>> = vec![];
    let msg = _create_cw721_transfer(&config, &recipient, token_id.clone());
    let msg_rs = match msg {
        Ok(msg) => msg,
        Err(ctr_err) => return Err(ctr_err),
    };
